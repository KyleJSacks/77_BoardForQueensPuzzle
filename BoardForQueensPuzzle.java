...
public class BoardForQueensPuzzle {
    ...
    public boolean lastIsNg() {
        ...
        int newQueen = filesWithQueens[lastRankFilled];
		for(int i = 0; i < lastRankFilled; i++){
			if(filesWithQueens[i] == newQueen) return true;
			if(newQueen + Math.abs(lastRankFilled - i) < this.ranks()){
				if(newQueen + Math.abs(lastRankFilled - i) == filesWithQueens[i]){
					return true;
				}
			}
			if(newQueen - Math.abs(lastRankFilled - i) >= 0){
				if(newQueen - Math.abs(lastRankFilled - i) == filesWithQueens[i]){
					return true;
				}
			}
